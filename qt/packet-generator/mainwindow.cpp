...
void MainWindow::on_SaveL4Button_clicked() {
	...
	if (ui->checkbox_TCP_create->isChecked()) {
		...
		if (checkMTU() == false) {
			...
			box->setText("Provided text length exceeds MTU 1500 size.");
			box->exec();
			delete box;
			delete tcp_h;
			tcp_h = NULL;
			cleanTable(ui->layer4_tableWidget);
			ui->checkbox_TCP_create->setChecked(false);
			ui->TCP_groupBox->setDisabled(true);
		} else {
			...
			int full_length = (20 + 20
					+ ui->textEdit_tcp_data->toPlainText().length()); //IP header length = 20 and TCP header length = 20;
		...
		if (checkMTU() == false) {
			...
			udp_h = NULL;
			cleanTable(ui->layer4_tableWidget);
			ui->checkbox_UDP_create->setChecked(false);
			ui->UDP_groupBox->setDisabled(true);
		} else {
			...
			QString udp_length_final_string = QString::fromStdString(
					udp_length_string);
			...
			int full_length = (20 + 8
					+ ui->textEdit_udp_data->toPlainText().length()); //IP header length = 20 and UDP header length = 8;
			std::string length_string = std::to_string(full_length);
			QString final_string = QString::fromStdString(length_string);
			ui->lineEdit_ip_total_length->setText(final_string);
		...
		QMessageBox* box = new QMessageBox();
		box->setText(
				"You must choose between TCP or UDP packet form in order to save Layer 4");
		box->exec();
		delete box;
	}
...
//    socket->buff_layer4 = NULL;
	...
	thread->start();
	emit start(this, socket, ip_h, tcp_h, udp_h, to_send_ip, ip_cksm,
			to_send_tcp, tcp_cksm, to_send_udp, udp_cksm);
...
void MainWindow::fillIPTable() {
	...
	QTableWidgetItem* ttl = new QTableWidgetItem(
			ui->lineEdit_ip_ttl->text().toStdString().c_str());

	QTableWidgetItem* protocol = new QTableWidgetItem(
...
void MainWindow::fillTCPTable() {
	...
	if (ui->checkBox_tcp_rand_src_port->isChecked() == true) {
		...
		source_port = new QTableWidgetItem(
				ui->lineEdit_tcp_src_port->text().toStdString().c_str());
	}
	...
	if (ui->checkBox_tcp_rand_dest_port->isChecked() == true) {
		...
		destination_port = new QTableWidgetItem(
				ui->lineEdit_tcp_dest_port->text().toStdString().c_str());
	}
	...
	if (ui->checkBox_tcp_rand_seq_number->isChecked() == true) {
		...
		seq_num = new QTableWidgetItem(
				ui->lineEdit_tcp_seq_num->text().toStdString().c_str());
	}
	...
	if (ui->checkBox_tcp_rand_ack_num->isChecked() == true) {
		...
		ack_num = new QTableWidgetItem(
				ui->lineEdit_tcp_ack_num->text().toStdString().c_str());
	}

	QTableWidgetItem* data_offset = new QTableWidgetItem(
			ui->lineEdit_tcp_data_offset->text().toStdString().c_str());
	...
	QTableWidgetItem* window_size = new QTableWidgetItem(
			ui->lineEdit_tcp_window_size->text().toStdString().c_str());
	...
	QTableWidgetItem* urg_pointer = new QTableWidgetItem(
			ui->lineEdit_tcp_urgent_pointer->text().toStdString().c_str());
	QTableWidgetItem* options = new QTableWidgetItem("0");
...
void MainWindow::fillUDPTable() {
	...
	if (ui->checkBox_udp_rand_src_port->isChecked() == true) {
		...
		source_port = new QTableWidgetItem(
				ui->lineEdit_udp_src_port->text().toStdString().c_str());
	}
	...
	if (ui->checkBox_udp_rand_dest_port->isChecked() == true) {
		...
		destination_port = new QTableWidgetItem(
				ui->lineEdit_udp_dest_port->text().toStdString().c_str());
	}
	...
	QTableWidgetItem* data = new QTableWidgetItem(
			ui->textEdit_udp_data->toPlainText());
...
void MainWindow::setValidators() {
	...
	validMTU = new QIntValidator(58, 65535);
	...
	ui->lineEdit_MTU->setValidator(validMTU);
...
bool MainWindow::checkMTU() {
	int MTU = ui->lineEdit_MTU->text().toInt();
	if (udp_h != NULL) {
		if (vlan_h != NULL) {
			MTU -= 18;
		} else {
		...
		MTU -= 28; //IPv4 header + TCP header sizes
		if (ui->textEdit_udp_data->toPlainText().length() > MTU) {
			return false;
		} else {
			return true;
		}
		...
		MTU -= 40; //IPv4 header + UDP header sizes
		if (ui->textEdit_tcp_data->toPlainText().length() > MTU) {
			return false;
		} else {
			return true;
		}
